...
* $P_X(\lbrace x \rbrace) = 0$ for all $x \in \mathcal{X}$, while $f_X(x) > 0$ for some $x \in \mathcal{X}$. For example, if we consider a Gaussian random variable with mean $\mu$ and variance $\sigma^2 = 0.1$, then the distribution value of $\lbrace 0 \rbrace$ is $P_X(\lbrace 0 \rbrace) = 0$, while the pdf at that point is $f_X(0) \approx 1.26$. This is because the probability is highly concentrated around the mean (it has high _density_), however, the probability of _exactly_ the mean is 0.
...
* Then, instead of writing something like: "We learn the distribution $p(Y|X)$ ..." you can write some of the following:
  * "We learn the distribution described by the density $p(Y|X)$."
  * "We learn the distribution described by $p(Y|X)$."
  * "We learn the density $p(Y|X)$."
